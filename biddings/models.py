...
class BiddingManager(models.Manager):
    ...
    def new_and_update(self, request, slug=None):
        ...
        qs = self.get_queryset().filter(user=user, product_item=product_item_obj)
        ...
        if request.user.is_authenticated: # 로긴한것과 activate 티켓있는지 확인.나중에는 일정등급 user인지만 확인하게하자. 
            if qs.exists():
                ...
                bidding_obj = self.new(user=user, product_item=product_item_obj)
    ...
    def new(self, user, product_item):
        user_obj = user
        if user is not None:
            if user.is_authenticated:
                user_obj = user
        return self.model.objects.create(user=user_obj, product_item=product_item)
...
def bidding_pre_save_receiver(sender, instance, *args, **kwargs):
    ...
    # 세일레이쇼(사실상 당첨후 카트나 order에 표시할려고 필드추가함.)
    instance.sale_ratio = (instance.product_item.product.list_price - instance.bidding_price) / instance.product_item.product.list_price * 100

...
class TicketOrderManager(models.Manager):
    def new_or_get(self, billing_profile, ticketlist_obj):
        created = False
        qs = self.get_queryset().filter(billing_profile=billing_profile, cart=ticketlist_obj, active=True, status='created')
        if qs.count() == 1:
            obj = qs.first()
        else:
            obj = self.model.objects.create(billing_profile=billing_profile, cart=ticketlist_obj)
            created = True
        return obj, created


class TicketOrder(Order):
    cart            = models.ForeignKey(TicketList, on_delete=models.CASCADE) # 티켓구매 카트로 대체(TicketList 모델)
    shipping_total  = models.DecimalField(default=0, max_digits=100, decimal_places=2) # 필요없다.
    # 배송되는것이 아니기 때문에 아래 두개도 딱히 필요는 없다.
